...
topic_segmentation_system_prompt = Template("\n".join([
    ...
    "",
    "For each sub-topic (ONLY if the topic is supported), you MUST provide these exactly 4 fields:",
    "- 'id': a short unique identifier as a string (\"1\", \"2\", \"3\", ...). MANDATORY.",
    "- 'title': the sub-topic name.",
    "- 'search_query': simple core technical keywords optimized for searching a course database.",
    "- 'dependsOn': an array of ids of the sub-topics that are DIRECT prerequisites (must be learned first). Use [] for foundational topics with no prerequisites. MANDATORY.",
    "",
    ...
    "",
    "Return ONLY a valid JSON array of objects (if supported) or a JSON object (if unsupported). No conversational text. Do NOT omit 'id' or 'dependsOn'.",
...
topic_segmentation_user_prompt = Template("\n".join([
    ...
    "CRITICAL: You MUST include the 'id' and 'dependsOn' fields for EVERY single sub-topic. Do NOT omit them."
]))
