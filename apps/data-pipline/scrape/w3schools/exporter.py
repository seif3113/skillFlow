"""
Data Exporter module for saving scraped W3Schools data.
"""
...
class DataExporter:
    """Handles exporting scraped W3Schools data to various formats."""
    ...
    def export_to_csv(
        ...
        if not filename:
            ...
            filename = f"w3schools_courses_{timestamp}.csv"
        ...
        with open(filepath, mode, newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(
                f, fieldnames=W3SCHOOLS_FIELDNAMES, extrasaction="ignore"
            )
            if write_header:
                writer.writeheader()
    ...
    def export_to_json(
        ...
        if not filename:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"w3schools_courses_{timestamp}.json"
...
class StreamingCSVExporter(_BaseStreamingCSVExporter):
    """W3Schools streaming CSV exporter with pre-configured fieldnames."""

    def __init__(self, filepath: str):
        super().__init__(filepath, fieldnames=W3SCHOOLS_FIELDNAMES)
