#!/usr/bin/env python
"""
Coursera Course Scraper - Main Entry Point  (Selenium edition)
...
from coursera.proxy_manager import ProxyManager
from coursera.exporter import DataExporter, StreamingCSVExporter
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("coursera_scraper.log", encoding="utf-8"),
    ],
)
...
def list_categories() -> None:
    print("\n=== Available Coursera Categories ===\n")
    for slug, name in CATEGORIES.items():
        print(f"  {slug:40} - {name}")
    print("\n=== Data Science Subcategories ===\n")
    for slug, name in DATA_SCIENCE_SUBCATEGORIES.items():
        print(f"  {slug:40} - {name}")
    print("\n=== Computer Science Subcategories ===\n")
    for slug, name in COMPUTER_SCIENCE_SUBCATEGORIES.items():
        print(f"  {slug:40} - {name}")
    print("\nUsage: python main.py --category <slug> [--subcategory <slug>]")
    print("Example: python main.py --category data-science --subcategory machine-learning\n")
...
def scrape_category(args: argparse.Namespace) -> None:
    ...
    min_pct = args.min_percentage / 100.0 if args.min_percentage else MIN_PAGES_PERCENTAGE
    ...
    output_file = (
        args.output
        or f"coursera_{category}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    )
    ...
    if ml_format and courses_collected:
        ml_file = DataExporter(OUTPUT_FOLDER).export_for_ml(
            courses_collected,
            f"coursera_{category}_ml_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
        )
    ...
    if use_proxies:
        logger.info(f"Proxies alive : {stats['available_proxies']}/{stats['total_proxies']}")
...
def scrape_search(args: argparse.Namespace) -> None:
    ...
    output_file = (
        args.output
        or f"coursera_search_{query.replace(' ', '_')}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Coursera Selenium Scraper — extract course data for ML training",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python main.py --category data-science --max-pages 20
  python main.py --category computer-science --subcategory algorithms
  python main.py --category business -o business.csv --show-browser
  python main.py --search "deep learning" --max-pages 10
  python main.py --category data-science --ml-format
  python main.py --list-categories
        """,
    )
    ...
    parser.add_argument(
        "--min-percentage", type=float, default=50.0, help="Min %% of pages (default 50)"
    )
    parser.add_argument(
        "--details", "-d", action="store_true", help="Fetch full course details (slower)"
    )

    parser.add_argument("--no-proxies", action="store_true", help="Disable proxy rotation")
    parser.add_argument("--proxy-file", type=str, help="File with proxy list (ip:port per line)")

    parser.add_argument("--output", "-o", type=str, help="Output CSV filename")
    parser.add_argument("--ml-format", action="store_true", help="Also export ML-ready CSV")

    parser.add_argument(
        "--show-browser", action="store_true", help="Show Chrome window (disable headless)"
    )
    parser.add_argument(
