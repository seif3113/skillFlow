"""
Data Exporter module for saving scraped YouTube data.
"""
...
class DataExporter:
    """Handles exporting scraped YouTube data to various formats."""
    ...
    def export_to_csv(
        ...
        if not filename:
            ...
            filename = f"youtube_courses_{timestamp}.csv"
        ...
        with open(filepath, mode, newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(
                f, fieldnames=YOUTUBE_FIELDNAMES, extrasaction="ignore"
            )
            if write_header:
                writer.writeheader()
    ...
    def export_to_json(
        ...
        if not filename:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"youtube_courses_{timestamp}.json"
...
class StreamingCSVExporter(_BaseStreamingCSVExporter):
    """YouTube streaming CSV exporter with pre-configured fieldnames."""

    def __init__(self, filepath: str):
        super().__init__(filepath, fieldnames=YOUTUBE_FIELDNAMES)
